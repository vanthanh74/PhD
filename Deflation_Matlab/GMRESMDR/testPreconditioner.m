...
% x_tilde = gmres(lhs,rhs,200,1e-6,800); %gmres(A,b,restart,tol,maxit,M1,M2,x,varargin)
...
% [x_tilde,flag,relres,iter,resvec] = cgs(A_kND,b);
%--------------------------------------------------------------------
%BFGMRES
% x_tilde = BFGMRES((I-Q2Q2t   + Q2*R22^-1*Q2' )*A_kND,(I-Q2Q2t + Q2*R22^-1*Q2' )*b,tol,maxit);
% x_tilde_exact = ((I-Q2Q2t   + Q2*R22^-1*Q2' )*A_kND)\((I-Q2Q2t + Q2*R22^-1*Q2' )*b);
% norm(x_tilde-x_tilde_exact)
%--------------------------------------------------------------------
%BFRRGMRES
% x_tilde = BFRRGMRES(lhs,rhs,tol,maxit);
...
r_0 = rhs0 - lhs0 * x_0; % b-A*x_0
% r_0 = rhs - lhs * x_0; % b-A*x_0

normr = norm(r_0);
V(:, 1) = r_0/normr;%(A1*rhs)/norm(A1*rhs);    % GMRES
...
%scaling 
[A_kND_scaling,R00,C00] = equil_rar(A2);
...
x_1 = x_0 + V(:, 1 : maxit) * y;
relres = norm(b-A_kND_scaling*x_1)/norm(b)
% r_1 = V * (c - H * y);
% x_tilde = x_1;
% normX_Xexact = norm(x_exact-x_1)
[x_gmres1,flag1,relres1,iter1,resvec1]=gmres(lhs0,rhs0,[],1e-6,maxit);
...
[x_gmres_scaling,flag_scaling,relres_scaling,iter_scaling,resvec_scaling] = gmres(A_kND_scaling,b,[],1e-6,maxit,M);  %
