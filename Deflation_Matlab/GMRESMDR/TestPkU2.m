...
V2_tilde = P_C*[-pinv(R11)*R12;eye(k)];  % size n x k
Q2Q2t = Q2*Q2';
Q2tQ1 = Q2'*Q1;
PctPc=P_C'*P_C;
PcPct=P_C*P_C';
zerosIdentity=pinv(R22)*Q2'*A_kND*P_C;   % [0 I] size k x n
...
% x_tilde = gmres(lhs,rhs); % gmres(A,b,restart,tol,maxit,M1,M2,x,varargin)
...
%--------------------------------------------------------------------
%BFGMRES
% x_tilde = BFGMRES(lhs,rhs,tol,maxit);
...
% x_gmres=gmres(A,b);
% normXgmres_Xexact = norm(x_exact-x_gmres)
%-------------------------------------------------------
