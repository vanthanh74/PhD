...
[p1, edgecut, s, begin_in, end_in] = DefineAlpha(A_kND, nb_subdomains);


%scaling 
[A_scaling,R00,C00] = equil_rar(A_kND);
...
M = @(x) preconditioner(Q2,R22,x) + blcLS(Pr, begin_in, end_in, x)  ;
% M1 = (I-Q2Q2t + Q2*R22^-1*Q2');
[x,flag,relres,iter,resvec] = gmres(A1,b,[],1e-6,50,M1);  %
relres= relres
iter= iter(2)
