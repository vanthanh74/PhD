...
normr = norm(r_0);
V(:, 1) = (A1*rhs)/norm(A1*rhs); %r_0/normr;
e_1 = zeros(maxit + 1, 1);
e_1(1) = 1;
c = normr * e_1;
resvec=[normr];
...
% %-------------------------------------------------------
%svd deflation
[U,Sigma,V] = svd(full(A_kND));
U2 = U(:,end-k+1:end);
Sigma2 = Sigma(end-k+1:end,end-k+1:end);
V2 = V(:,end-k+1:end);

%solve (I-U2U2')Ax = (I-U2U2')b for x_tilde
% I_k = I(1:k,1:k);
x_tilde_svd = ((I-U2*U2')*A_kND)\((I-U2*U2')*b);
x_svd = (I-V2*V2')*x_tilde_svd + V2*Sigma2^-1*U2'*b;
norm(x_exact-x_svd)
...
x_tilde_qr = ((I-Q2*Q2')*A_kND)\((I-Q2*Q2')*b);
x_qr = (I-V2_tilde*zerosIdentity*P_C')*x_tilde_qr  +  V2_tilde*R22^-1*Q2'*b;
norm(x_exact-x_qr)
