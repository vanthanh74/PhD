...
function [a,aa,x,resvec,r,nmv,relres] = lgcrodr(A,b,m,k,x0,tol,M1,M2,reuse_name)
...
if(isfield(U_persist,reuse_name))
   ...
   [x,r,V,H,p,resvec_inner,y] = mgmres1(A,x,r,m,M1,M2,tol*bnorm);
...
while(resvec(nmv) / bnorm > tol)
  ...
  if mod(iter,5)==0                    %
      BB(:,iter)=r/norm(r);
      a(ceil((iter-1)/10))=acos(BB(:,iter)'*BB(:,iter-2))*h;%
      aa(ceil((iter-1)/10))=acos(BB(:,iter)'*BB(:,iter-1))*h;%
  end  
   
   % xlabel('xlabel be ceil((s-1)/10),with mod(s,5)=0 and s be the number of restarts')
   
  %ylabel('Size of the seq. and skip angles')
