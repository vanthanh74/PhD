...
L2NormError = sqrt(sum(Dt*(uF_true(pick)-U).^2));%norm(uF_true(pick)-U,2);%
LInfNormError = norm(uF_true(pick)-U,inf);%max(abs(uF_true(pick)-U));%
...
%         U = M_tilde\(M_tilde*U + (f-A_tilde*U));
        U = U+M_tilde\(f -  A_tilde*U); %use
%         U = U+M_tilde\(f -  A_tilde*U);
...
%        norm(uF_true(pick)-U,'inf')
       ...
       L2NormError(k+1) = sqrt(sum(Dt*(uF_true(pick)-U).^2));%norm(uF_true(pick)-U,2);%
       LInfNormError(k+1) = norm(uF_true(pick)-U,inf);%max(abs(uF_true(pick)-U));%
...
% legend('L^{\infty}NormError','Superlinear bound','Linear bound')
% xlabel('k')
% ylim([1e-18 1])
title(['T=',num2str(T)])
