...
N=8;% parareal parameters
K=10; u_init=[1 1 1 1 1 1]; T=10*2*pi/c;  
...
% G=@(t0,t1,u0) SGRunge_Kutta4_Exp_spiral_6D_Const(fG1,t0,t1,u0,MG); %  Limit model coarse solver G coarse solver G
...
legend({'L^{2}-norm','L^{\infty}-norm'},'location','southwest','fontsize',15)
title(['Convergence rate with N = ', num2str(N),' and MG = ',num2str(MG)],'fontsize',15)
xlabel('Iteration')
ylabel('Error')
