...
n  = 13; % number of nodes    5-3,7-4
N_subdomains = (n-1)/3+1;

A  = -phi*diag(diag(eye(n-1)),-1) + eye(n)

B  = -phiDT*diag(diag(eye((n-1)/3)),-1) + eye((n-1)/3+1)

B1 = -phiDT*diag(diag(eye((n-1)/3)),-1)
...
for i = 2: N_subdomains 
    R_sub{i} = R2_end(N_nodes_subdomain*(i-2)+1:N_nodes_subdomain*(i-2)+3,:);
...
eig_F = eig(F)
eig_C = eig(C)
FC_bound = max(abs(eig(F).^2 - eig(C)).*(1 - abs(eig(C)).^(19/2) )./ (1 - eig(C)))

FCF_bound = max(abs(eig(F).^2 - eig(C)).*(1 - abs(eig(C)).^(19/2) )./ (1 - eig(C)).*abs(eig(F)).^2)