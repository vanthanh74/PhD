...
type = 'H'; % Heat 'H',  Convection-diffusion 'CD'
...
%-------------------------------------------------------
% % exact solution 2
u_ex=@(x,t) sin(pi*x)*exp(-2*t);%x*(L-x)^2*exp(-2*t);
...
L2NormError = [ sqrt(sum(dX*sum(dT*(U_fine_mat_reduce' - uF_seq_mat').^2)))];%
...
%     L2NormError(k+1) = [norm(U_fine_mat_reduce' - uF_seq_mat',2)];%sqrt(sum(sum(dT*(uF_seq(end-nx_coarse+2:end,:)-U(end-nx_coarse+2:end,:)).^2)));%
   L2NormError(k+1) = sqrt(sum(dX*sum(dT*(U_fine_mat_reduce' - uF_seq_mat').^2)));%sqrt(sum(sum(dT*(uF_seq(end-nx_coarse+2:end,:)-U(end-nx_coarse+2:end,:)).^2)));%
    ...
    FC_boundL2(k+1) = max(abs(eig(phi).^m - eig(phiDT)).*(1 - abs(eig(phiDT)).^((nt_interval-1)/m) )./ (1 - abs(eig(phiDT)))).^(k) *sqrt(sum(dX*sum(dT*(U_fine_mat_reduce' - uC_mat').^2)));
...
E = [E; max(abs(eig(phi).^m - eig(phiDT)).*(1 - abs(eig(phiDT)).^((nt_interval-1)/m) )./ (1 - abs(eig(phiDT)))).^(k)]
...
figure
semilogy(0:K-1,L2NormError,'b--o',0:K-1,FC_boundL2,'k-*')
