...
/********************************
TO MAKE IT WORK ON 64bit MACHINES

* include the MATLAB "matrix.h"
* in convertMatrix function, declare *jc and *ir as mwIndex instade of int
  (line 109)
* compile with -largeArrayDims mex option
********************************/




#include "/Applications/MATLAB_R2017b.app/extern/include/mex.h"
#include "/Applications/MATLAB_R2017b.app/extern/include/matrix.h"
#include <strings.h>
#include "metis.h"
...
{
    int i, n, nparts, wgtflag, options[8] = {0}, numflag = 0, edgecut, sepsize, npes;
    idxtype *xadj, *adjncy, *vwgt, *adjwgt, *part, *perm, *iperm, *sep, *sizes; /* Sizes added by Pawan on May 6, 2009 to know the sizes of separators.*/
    char funcname[FUNCNAMELEN];
    double *optarray, *partpr, *permpr, *ipermpr, *seppr, *sizespr;

    /* First do some general argument checking */
   /*  if (nrhs < 2 || nrhs > 5 || nlhs > 2) { Commented by Pawan on May 06, 2009. */

    if (nrhs < 2 || nrhs > 5 || nlhs > 3) {  /* Added by Pawan to validate one more output (sizes in METIS_NDP), On May 06, 2009. */
    ...
    if (strcasecmp(funcname,"PartGraphRecursive")==0) {
    ...
    } else if (strcasecmp(funcname,"PartGraphVKway")==0) {
        ...
        /* Do the call */
        METIS_PartGraphVKway (&n, xadj, adjncy, vwgt, adjwgt, &wgtflag,
			      &numflag, &nparts, options, &edgecut, part);
    ...
    } else if (strcasecmp(funcname,"NodeNDP")==0) {

        if (nrhs < 3) {
            mexErrMsgTxt ("Third parameter needed: number of parts");
        }
        
    
        /* Figure out values for options */
       /* if (nrhs >= 3) { */        
          if (nrhs >= 4) {  /* Added by Pawan on May 6, 2009 */         
            optarray = mxGetPr (NDOPTS_IN);
            for (i = 1; i < 4; ++i) {
                options[i] = (int) optarray[i-1];
            }
        ...
        perm =  (idxtype*) mxCalloc (n, sizeof(idxtype));
        iperm = (idxtype*) mxCalloc (n, sizeof(idxtype));
        npes = (int) mxGetScalar (NPES_IN);
        sizes = (idxtype*) mxCalloc (2*npes-1, sizeof(idxtype)); /* Added by pawan on May 6, 2009, sizes required in METIS_NodeNDP function. */
        
        
	/* Do the call */
      /*  METIS_NodeND (&n, xadj, adjncy, &numflag, options, perm, iperm);  Commented by pawan on May 6, 2009. */
        
	METIS_NodeNDP (n, xadj, adjncy, npes, options, perm, iperm, sizes); /* Added by Pawan on May 6, 2009. */
        ...
        if (nlhs >= 1) {
            ...
            if (nlhs >= 2) {
            	...
            	if (nlhs >= 3) {          /* Five lines added by Pawan on May 06, 2009. Do I need to increase the sizes by 1 ??? */
                	SIZES_OUT = mxCreateDoubleMatrix (1, 2*npes-1, mxREAL);
                	sizespr = mxGetPr (SIZES_OUT);
                	for (i = 0; i < 2*npes-1; i++) {
                    		sizespr[i] = sizes[i]; /* Should not add 1 to sizes , To be verified ! */
                    }	
            	}
            }
	}
