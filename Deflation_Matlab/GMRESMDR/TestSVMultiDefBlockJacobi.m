...
% A0 =  mmread(matrix);
MinvA = defpreconditioner(Q2,R22,maxSV,A);%;%(I-Q2*Q2' +  Q2*(R22^-1)*Q2')*A;%
...
MinvA1 = defpreconditioner(Q2,R22,maxSV,blcLS(Pr, begin_in, end_in, A)); % (M1^-1M2^-1)A
% MinvA1 = defpreconditioner(Q2,R22,A) + blcLS(Pr, begin_in, end_in,A) - blcLS(Pr, begin_in, end_in,A)*defpreconditioner(Q2,R22,A);
% MinvA1 = defpreconditioner(Q2,R22,A) + blcLS(Pr, begin_in, end_in,A);
...
dd=1
compare_mat=[sA_k sR_k sR22_k s_k s_k1 s_k2]
figure
subplot(131)
spy(MinvA) % def
subplot(132)
