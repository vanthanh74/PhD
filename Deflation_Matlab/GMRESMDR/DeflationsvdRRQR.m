function [P_k,Y_k,C_k,U_k,resvec] = DeflationsvdRRQR(A,b,matInd,methodInd,methodDef,resvec,adaptive_deflation,V,H,k,cycle,maxit,maxcycle,x_0,x_1,r_0,maxdef,tol,method)
n=size(A);
if(k > 0)
...
%                 [Q, R] = qr(A * blcLS(Pr, begin_in, end_in, Y_k), 0);
                ...
                %         U_k = Y_k / Sigma_k(end - k : end - 1, end - k + 1 : end);
                 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %%%%  Singular values RRQR %%%%%%%%%%%
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            elseif(strcmp(method, 'svdrrqr') == 1)
%                 [AP_k, Sigma_k, P_k] = svd(H,0);
                [Q_final,R_final,P_C] = qr(H,'matrix'); 
%                 P_k = P_k(:, end - k + 1 : end);
                Q1=Q_final(:,1:maxit-k);  % size n x n-k
                Q2=Q_final(:,end-k+1:end); % size n x k
                R11=R_final(1:maxit-k,1:maxit-k); % size n-k x n-k
                R12=R_final(1:maxit-k,maxit-k+1:end); % size n-k x k
                R22=R_final(end-k+1:end,end-k+1:end); % size k x k
%                 normR22=norm(R22)
                %%Approximate for the right null space of A
                P_k = P_C*[-(R11^-1)*R12;eye(k)];  % siz
