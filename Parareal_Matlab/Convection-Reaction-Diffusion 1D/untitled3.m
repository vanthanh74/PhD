...
Uk_0 = zeros((nt_fine+1)*(nx_fine-1),1);
Uk_0(1:nx_fine-1) = ux0;
rhs0 = [ux0; zeros(((nt_fine+1)*(nx_fine-1)-nx_fine+1),1)];
...
%     i_temp = 1;
    for i = 2:nt_fine+1
        F(i_temp*(nx_fine-1) + 1 :i_temp*(nx_fine-1) + nx_fine-1,:) = A_F^-1*dt*f(xF(2:nx_fine),tF(i))';
        i_temp = i_temp + 1;
    end
