function DU = UpdateConvDiffFCF(FF,U_old)
...
%         i = i + 1
  ...
  % second level ~ coarse grid correction sequentially
    ...
    DU_temp =   U_old_temp_4;
    
 
    
  % one more additive Schwarz
     U_old_update = U_old + DU_temp;
     r_k_update = FF - A_Original*U_old_update;
    for i = 1:N_subdomain+1
%         i = i + 1
        if i == 1
            x_temp_3 = R1*r_k_update;
            x_temp_4 = A1\x_temp_3;
            x_update{i} = R1'*x_temp_4;
            DU = x_update{i};         
        else
            x_temp_3 = R_sub{i}*r_k_update;
            x_temp_4 = A_sub{i}\x_temp_3;
            x_update{i} = R_sub{i}'*x_temp_4;  
            DU = DU_temp + DU + x_update{i};
        end            
    end
