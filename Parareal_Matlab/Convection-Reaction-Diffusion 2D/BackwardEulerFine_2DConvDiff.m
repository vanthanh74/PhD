function [U1,nmv_total] = BackwardEulerFine_2DConvDiff(kappa,dx_fine,dy_fine,dt,f,u0,t0,nx_fine,ny_fine,nt,name)
...
r1 = kappa*dt/(dx_fine^2);
k1 = p1*dt/dx_fine;

r2 = kappa*dt/(dy_fine^2);
k2 = p2*dt/dy_fine;
...
A = -(r1+k1)*diag(ones((ny_fine-1)*(ny_fine-1) - 1,1),-1) ...
    + (1+2*r1+2*r2 - p3*dt)*eye((ny_fine-1)*(ny_fine-1)) ...
...
for  n = 1 : nt
    ...
    for i=1:nx_fine-1
        for j=1:ny_fine-1
            F((i-1)*(nx_fine-1) +j)= dt*f(i*dx_fine,j*dy_fine,t0);
        end
    end
