function  [nmv_total_matrix_FineSolver,nmv_total_matrix_FineSolverInParareal,TimeConsumingFineSolver,TimeConsumingParareal,total_nmv_perIterParareal,total_nmv] = parareal_2d_convection_diffusion_General(L,T,kappa,dT,dX_coarse,dY_coarse,...
...
nameC = strseq('Csubspace',1:nt_coarse);
nameC = char(nameC);
for i =1 :nt_coarse
     if nt_coarse >= 10 && nt_coarse <= 99 && i <=9
          [U,~] = BackwardEuler_2DConvDiff(kappa,dX_coarse,dY_coarse,dT,f,u0_restriction,tC(i),nx_coarse,ny_coarse,M,[]);
    elseif nt_coarse >= 10 && nt_coarse <= 99 && i >=10
           [U,~] = BackwardEuler_2DConvDiff(kappa,dX_coarse,dY_coarse,dT,f,u0_restriction,tC(i),nx_coarse,ny_coarse,M,[]);
     end   
%     U = BackwardEuler_2DConvDiff(kappa,dX_coarse,dY_coarse,dT,f,u0_restriction,tC(i),nx_coarse,ny_coarse,M,nameC);
    u0_restriction = U(end-nx_coarse:end,:);
    
    U_interp_temp = interpolation(xC,yC,u0_restriction,xF,yF);
    %     figure
...
for n = 1:nt_coarse
    if nt_coarse >= 10 && nt_coarse <= 99 && n <=9
          [U_Fine,nmv_total] = BackwardEuler_2DConvDiff(kappa,dx_fine,dy_fine,dt,f,U_Fine0,tC(n),nx_fine,ny_fine,m,name(n,1:end-1));
    elseif nt_coarse >= 10 && nt_coarse <= 99 && n >=10
           [U_Fine,nmv_total] = BackwardEuler_2DConvDiff(kappa,dx_fine,dy_fine,dt,f,U_Fine0,tC(n),nx_fine,ny_fine,m,name(n,:));
    end
...
normInfDefect = [norm(U_interpolation - U_FineSequential,inf)];%[max(max(abs(U_interpolation - U_FineSequential)))];%[norm(U_interpolation(end-nx_fine:end,:) - U_FineSequential(end-nx_fine:end,:),inf)];%
U0 = U_interpolation;
...
length_number = length(nameFineSolver(end,:)) - 12;
nameCoarseSolver = strseq('subspaceCoarse',1:nt_coarse*(K-1));
nameCoarseSolver = char(nameCoarseSolver);
...
% end
    ...
    for  n = 1 : nt_coarse
        % n = n + 1
        if nt_coarse*(k-1) + n <= 9 && length_number == 3
            [F0 ,nmv_toltal]=   BackwardEuler_2DConvDiff(kappa,dx_fine,dy_fine,dt,f,U_interpolation((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:),tC(n),nx_fine,ny_fine,m,nameFineSolver(nt_coarse*(k-1) + n,1:end-2));
        elseif nt_coarse*(k-1) + n >=10 && nt_coarse*(k-1) + n <= 99 && length_number == 3
            [F0 ,nmv_toltal]=   BackwardEuler_2DConvDiff(kappa,dx_fine,dy_fine,dt,f,U_interpolation((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:),tC(n),nx_fine,ny_fine,m,nameFineSolver(nt_coarse*(k-1) + n,1:end-1));
        elseif nt_coarse*(k-1) + n >=100 && nt_coarse*(k-1) + n <= 999 && length_number == 3
            [F0 ,nmv_toltal]=   BackwardEuler_2DConvDiff(kappa,dx_fine,dy_fine,dt,f,U_interpolation((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:),tC(n),nx_fine,ny_fine,m,nameFineSolver(nt_coarse*(k-1) + n,:)); 
        
        elseif nt_coarse*(k-1) + n <= 9 && length_number == 4
            [F0 ,nmv_toltal]=   BackwardEuler_2DConvDiff(kappa,dx_fine,dy_fine,dt,f,U_interpolation((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:),tC(n),nx_fine,ny_fine,m,nameFineSolver(nt_coarse*(k-1) + n,1:end-3));
        elseif nt_coarse*(k-1) + n >=10 && nt_coarse*(k-1) + n <= 99 && length_number == 4
            [F0 ,nmv_toltal]=   BackwardEuler_2DConvDiff(kappa,dx_fine,dy_fine,dt,f,U_interpolation((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:),tC(n),nx_fine,ny_fine,m,nameFineSolver(nt_coarse*(k-1) + n,1:end-2));
        elseif nt_coarse*(k-1) + n >=100 && nt_coarse*(k-1) + n <= 999 && length_number == 4
             [F0 ,nmv_toltal]=   BackwardEuler_2DConvDiff(kappa,dx_fine,dy_fine,dt,f,U_interpolation((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:),tC(n),nx_fine,ny_fine,m,nameFineSolver(nt_coarse*(k-1) + n,1:end-1));
        elseif nt_coarse*(k-1) + n >=1000 && nt_coarse*(k-1) + n <= 9999 && length_number == 4
             [F0 ,nmv_toltal]=   BackwardEuler_2DConvDiff(kappa,dx_fine,dy_fine,dt,f,U_interpolation((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:),tC(n),nx_fine,ny_fine,m,nameFineSolver(nt_coarse*(k-1) + n,:));
        end
...
%             [G0 ,~]=   BackwardEuler_2DConvDiff(kappa,dX_coarse,dY_coarse,dT,f,ux0F_restriction,tC(n),nx_coarse,ny_coarse,M,nameCoarseSolver(nt_coarse*(k-1) + n,1:end-2));
%         elseif nt_coarse*(k-1) + n >=10 && nt_coarse*(k-1) + n <= 99
%             [G0 ,~]=  BackwardEuler_2DConvDiff(kappa,dX_coarse,dY_coarse,dT,f,ux0F_restriction,tC(n),nx_coarse,ny_coarse,M,nameCoarseSolver(nt_coarse*(k-1) + n,1:end-1));
%         elseif nt_coarse*(k-1) + n >=100 && nt_coarse*(k-1) + n <= 999
%             [G0 ,~]=   BackwardEuler_2DConvDiff(kappa,dX_coarse,dY_coarse,dT,f,ux0F_restriction,tC(n),nx_coarse,ny_coarse,M,nameCoarseSolver(nt_coarse*(k-1) + n,:));            
%               end
         
            G0 =   BackwardEuler_2DConvDiff(kappa,dX_coarse,dY_coarse,dT,f,ux0F_restriction,tC(n),nx_coarse,ny_coarse,M,[]) ;
        G = [G;G0(end-nx_coarse:end,:)];
...
%     return
    ...
    if k < K
        for n = 1: nt_coarse
            ux0F_restriction = restriction(U_interpolation((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:),nx_coarse,ny_coarse,nx_fine,ny_fine);
            Gn = BackwardEuler_2DConvDiff(kappa,dX_coarse,dY_coarse,dT,f,ux0F_restriction,tC(n),nx_coarse,ny_coarse,M,[]) ;
            U_interpolation( (n)*(nx_fine+1)+1:(n+1)*(nx_fine+1),:) =     interpolation(xC,yC,Gn(end-nx_coarse:end,:),xF,yF)  ...
    ...
    normInf_U0_Uinterpolate = norm(U0-U_interpolation,inf) ;%max(max(abs(U0-U_interpolation))) ;%;norm(U0(end-nx_fine:end,:)-U_interpolation(end-nx_fine:end,:),inf) ;%
    normInf2executiveParearealIter =[normInf2executiveParearealIter;normInf_U0_Uinterpolate] ;
    normInfDefect =[normInfDefect ; norm(U_interpolation-U_FineSequential,inf)];%[normInfDefect ;max(max(abs(U_interpolation - U_FineSequential)))];%[normInfDefect ; norm(U_interpolation(end-nx_fine:end,:)-U_FineSequential(end-nx_fine:end,:),inf)]%
    U0 = U_interpolation;
end
normInf2executiveParearealIter
normInfDefect
