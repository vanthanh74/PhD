function  [nmv_total_matrix_FineSolver,nmv_total_matrix_FineSolverInParareal,TimeConsumingFineSolver,TimeConsumingParareal,total_nmv_perIterParareal,total_nmv] = testNonCoarsening(L,T,kappa,dT,dX_coarse,dY_coarse,...
    dt,dx_fine,dy_fine,nt_coarse,nx_coarse,ny_coarse,...
    nt_fine,nx_fine,ny_fine,m,K)


% IVP: u_t = kappa*(u_xx + u_yy) + f               in (0,L)x(0,L) x (0,T),
...
u0_temp = u0;
for i =1 :nt_coarse
    U_temp = BackwardEulerCoarse_2DConvDiff(kappa,dX_coarse,dY_coarse,dT,f,u0_temp,tC(i),nx_coarse,ny_coarse);    
    u0_temp = U_temp;
    U =[U;U_temp];
end
...
norm(U(end-nx_fine:end,:)  - U_exact,inf);

normInf2executedParearealIter = [];
normInfDefect = [norm(U(end-nx_fine:end,:) - U_FineSequential(end-nx_fine:end,:),inf)];
U0 = U;


figure
subplot(121)
surf(xF,yF,U(end-nx_fine:end,:))
zlim([0 3*1e-3])
xlabel('x')
ylabel('y')
zlabel('u')
...
for k = 1: K-1
    ...
    for  n = 1 : nt_coarse
        ...
        if nt_coarse*(k-1) + n <= 9
            [F0 ,nmv_toltal]=   BackwardEulerFine_2DConvDiff(kappa,dx_fine,dy_fine,dt,f,U((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:),tC(n),nx_fine,ny_fine,m,nameFineSolver(nt_coarse*(k-1) + n,1:end-2));
        elseif nt_coarse*(k-1) + n >=10 && nt_coarse*(k-1) + n <= 99
            [F0 ,nmv_toltal]=   BackwardEulerFine_2DConvDiff(kappa,dx_fine,dy_fine,dt,f,U((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:),tC(n),nx_fine,ny_fine,m,nameFineSolver(nt_coarse*(k-1) + n,1:end-1));
        elseif nt_coarse*(k-1) + n >=100 && nt_coarse*(k-1) + n <= 999
            [F0 ,nmv_toltal]=   BackwardEulerFine_2DConvDiff(kappa,dx_fine,dy_fine,dt,f,U((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:),tC(n),nx_fine,ny_fine,m,nameFineSolver(nt_coarse*(k-1) + n,:));            
        end
            G0 =   BackwardEulerCoarse_2DConvDiff(kappa,dX_coarse,dY_coarse,dT,f,U((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:),tC(n),nx_coarse,ny_coarse) ;
        G = [G;G0];
...
%     return
    ...
    if k < K
        for n = 1: nt_coarse
            Gn = BackwardEulerCoarse_2DConvDiff(kappa,dX_coarse,dY_coarse,dT,f,U((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:),tC(n),nx_coarse,ny_coarse) ;
            U( (n)*(nx_fine+1)+1:(n+1)*(nx_fine+1),:) =     Gn  - G((n-1)*(nx_coarse+1)+1:n*(nx_coarse+1),:)+ F((n-1)*(nx_fine+1)+1:n*(nx_fine+1),:);                
        end
    end
    
    
    normInf_U0_Uinterpolate = norm(U0(end-nx_fine:end,:)-U(end-nx_fine:end,:),inf) ;
    normInf2executedParearealIter =[normInf2executedParearealIter;normInf_U0_Uinterpolate] ;
    normInfDefect =[normInfDefect ; norm(U(end-nx_fine:end,:)-U_FineSequential(end-nx_fine:end,:),inf)];
    U0 = U;
end
normInf2executedParearealIter
normInfDefect
...
figure
surf(xF,yF,real(U(end-nx_fine:end,:)))
zlim([0 3*1e-3])
xlabel('x')
ylabel('y')
zlabel('u')
