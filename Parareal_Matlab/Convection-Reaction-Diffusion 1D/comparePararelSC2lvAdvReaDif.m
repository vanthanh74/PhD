...
%---------------------------------------------------------------------------------------------------
uF_seq_Parareal = A_tilde\( R_rhs*F);
uF_seq_NonO2 =  FineSolver(A_F,m,nt_interval,nx_fine,F);

uF_seq_Parareal_mat=[];
uF_seq_NonO2_mat = [];
for i = 1:length(uF_seq_NonO2)/(nx_coarse-1)
    uF_seq_Parareal_mat(:,i) = [uF_seq_Parareal((i-1)*(nx_coarse-1)+1:(i)*(nx_coarse-1))];
    uF_seq_NonO2_mat(:,i) = [uF_seq_NonO2((i-1)*(nx_coarse-1)+1:(i)*(nx_coarse-1))];
end
...
% L2NormError_NonO2 = [ norm(U_fine_mat_reduce' - uF_seq_mat',2)];%[ sqrt(sum(sum(dT*(uF_seq(end-nx_coarse+2:end,:)-uC(end-nx_coarse+2:end,:)).^2)))];%
% LInfNormErrorNonO2 = [norm(U_fine_mat_reduce' - uF_seq_mat',inf)];%[max(abs(uF_seq(end-nx_coarse+2:end,:)-uC(end-nx_coarse+2:end,:)))];%
% 
% L2NormErrorParareal = [ norm(U_fine_mat_reduce' - uF_seq_mat',2)];
% LInfNormErrorParareal = [norm(U_fine_mat_reduce' - uF_seq_mat',inf)];%[max(abs(uF_seq(end-nx_coarse+2:end,:)-uC(end-nx_coarse+2:end,:)))];%


L2NormErrorNonO2 = [ norm(U_fine_mat_reduce' - uF_seq_NonO2_mat',2)];%[ sqrt(sum(sum(dT*(uF_seq(end-nx_coarse+2:end,:)-uC(end-nx_coarse+2:end,:)).^2)))];%
% L2NormErrorNonO2 = [sqrt(sum(dX*sum(dT*(U_fine_mat_reduce' - uF_seq_NonO2_mat').^2)))];%
LInfNormErrorNonO2 = [norm(U_fine_mat_reduce' - uF_seq_NonO2_mat',inf)];%[max(abs(uF_seq(end-nx_coarse+2:end,:)-uC(end-nx_coarse+2:end,:)))];%

L2NormErrorParareal = [ norm(U_fine_mat_reduce' - uF_seq_Parareal_mat',2)];
% L2NormErrorParareal = [sqrt(sum(dX*sum(dT*(U_fine_mat_reduce' - uF_seq_Parareal_mat').^2)))];%
LInfNormErrorParareal = [norm(U_fine_mat_reduce' - uF_seq_Parareal_mat',inf)];%[max(abs(uF_seq(end-nx_coarse+2:end,:)-uC(end-nx_coarse+2:end,:)))];%

% FC_boundL2 = L2NormErrorNonO2;
FC_boundL2 = [max(abs(eig(phi).^m - eig(phiDT)).*(1 - abs(eig(phiDT)).^((nt_interval)/m) )./ (1 - abs(eig(phiDT)))).^(0) *norm(U_fine_mat_reduce' - uF_seq_NonO2_mat',2)];%sqrt(sum(dX*sum(dT*(U_fine_mat_reduce' - uC_mat').^2)));
...
for k = 1: K
   ...
   % Parareal
      ...
      U_Parareal = U_Parareal  + M_tilde\(R_rhs*F -  A_tilde*U_Parareal);
%     U_Parareal = MultilevelParareal1DconvdiffSol(T,L,a,b,c,nt_interval,nx_coarse,nt_coarse,nx_fine,nt_fine, ...
%                 solver,K,dx,dt,DT,dX,dT,M,m,u_ex ,ux0_function,f,xC,xF,tF,tC,rF,kF,rC,kC,U_Parareal);
   %-----------------------------------------------------------
...
%   % second level ~ coarse grid correction sequentially
    ...
    % Parareal solution
   U_Parareal_mat = [];
   for i = 1:length(U_Parareal)/(nx_coarse-1)
       U_Parareal_mat(:,i) = U_Parareal((i-1)*(nx_coarse-1)+1:(i)*(nx_coarse-1))';
   end   
   U_Parareal_reduce =  U_Parareal_mat;
   ...
   U_fine_mat_reduce =  U_fine_mat(:,pick);
    ...
    L2NormErrorNonO2(k+1) = [norm(U_fine_mat_reduce' - uF_seq_NonO2_mat',2)];%sqrt(sum(sum(dT*(uF_seq(end-nx_coarse+2:end,:)-U(end-nx_coarse+2:end,:)).^2)));%
%     L2NormErrorNonO2(k+1) = [ sqrt(sum(dX*sum(dT*(U_fine_mat_reduce' - uF_seq_NonO2_mat').^2)))];%
    LInfNormErrorNonO2(k+1) = [norm(U_fine_mat_reduce' - uF_seq_NonO2_mat',inf)];%max(abs(uF_seq(end-nx_coarse+2:end,:)-U(end-nx_coarse+2:end,:)));%
    
    L2NormErrorParareal(k+1) = [norm(U_Parareal_mat' - uF_seq_Parareal_mat',2)];%sqrt(sum(sum(dT*(uF_seq(end-nx_coarse+2:end,:)-U(end-nx_coarse+2:end,:)).^2)));%
%     L2NormErrorParareal(k+1) = [ sqrt(sum(dX*sum(dT*(U_Parareal_mat' - uF_seq_Parareal_mat').^2)))];%
    LInfNormErrorParareal(k+1) = [norm(U_Parareal_mat' - uF_seq_Parareal_mat',inf)];%max(abs(uF_seq(end-nx_coarse+2:end,:)-U(end-nx_coarse+2:end,:)));%
   
    
    FC_boundL2(k+1) = max(abs(eig(phi).^m - eig(phiDT)).*(1 - abs(eig(phiDT)).^((nt_interval)/m) )./ (1 - abs(eig(phiDT)))).^(k) *norm(U_fine_mat_reduce' - uC_mat',2);%sqrt(sum(dX*sum(dT*(U_fine_mat_reduce' - uC_mat').^2)));
...
figure
semilogy(0:K,LInfNormErrorNonO2,'r-^',0:K,L2NormErrorNonO2,'b-o')
...
figure
semilogy(0:K,L2NormErrorNonO2,'b--o',0:K,FC_boundL2,'m-*')
...
legend({'L^{\infty}- Parareal','L^2- Parareal ','L^2- SC two-level additive Schwarz','L^{\infty}- SC two-level additive Schwarz','SC bound L^2- norm'},'Location','northeast','FontSize',12) %northeast southwest
xlabel('Iteration ')
ylabel('Error')
% ylim([1e-18 1e-2])
ylim([1e-18 1e0])
