...
nt_interval = 3; % number of time intervals

nx_coarse = 10; % number of coarse spatial intervals
nt_coarse = 3;% 300 %20    % number of coarse time intervals


nx_fine = 	10; % number of fine spatial intervals
nt_fine = 12;% 3000 %200 . % number of fine time intervals (nt_coarsex2 )
...
normInfDefect = [norm(uC - uF_seq,inf)];%[norm(uC(end-nx_coarse+2:end,:) - uF_seq(end-nx_coarse+2:end,:),inf)];%

L2NormError = [ norm(uC - uF_seq,2)];%[ sqrt(sum(sum(dT*(uF_seq(end-nx_coarse+2:end,:)-uC(end-nx_coarse+2:end,:)).^2)))];%
LInfNormError = [norm(uC - uF_seq,inf)];%[max(abs(uF_seq(end-nx_coarse+2:end,:)-uC(end-nx_coarse+2:end,:)))];%
...
%     U = gmres(M_tilde,((M_tilde - A_tilde)*U + R_rhs*F),20,1e-16);%
    ...
    %
    normInf_U0n = norm(U-U0n,inf) ;
    normInf2executiveParearealIter =[normInf2executiveParearealIter;normInf_U0n] ;
    normInfDefect =[normInfDefect ; norm(U-uF_seq,inf)];
    
    U0n = U;
    L2NormError(k+1) = [norm(U - uF_seq,2)];%sqrt(sum(sum(dT*(uF_seq(end-nx_coarse+2:end,:)-U(end-nx_coarse+2:end,:)).^2)));%
    LInfNormError(k+1) = [norm(U - uF_seq,inf)];%max(abs(uF_seq(end-nx_coarse+2:end,:)-U(end-nx_coarse+2:end,:)));%
...
U_plot = [zeros(nt_interval+1,1) U_plot zeros(nt_interval+1,1)];
