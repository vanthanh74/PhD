function [x,flag,relres,iter,resvec,L2NormErrorGMRES,LInfNormErrorGMRES] = mygmresConvDiff(A,b,restart,tol,maxit,M1,M2,x,varargin)
...
L2NormErrorGMRES = norm(U_fine_mat_reduce' - uF_seq_mat',2);
% L2NormErrorGMRES = [ sqrt(sum(sum((U_fine_mat_reduce' - uF_seq_mat').^2)))];%[ sqrt(sum(sum(dT*(uF_seq(end-nx_coarse+2:end,:)-uC(end-nx_coarse+2:end,:)).^2)))];%
...
%         end
        ...
        L2NormErrorGMRES(initer+1) = norm(U_fine_mat_reduce' - uF_seq_mat',2);%sqrt(sum(sum(dT*(uF_seq(end-nx_coarse+2:end,:)-U(end-nx_coarse+2:end,:)).^2)));%    
%         L2NormErrorGMRES(initer+1) = sqrt(sum(sum((U_fine_mat_reduce' - uF_seq_mat').^2)));%sqrt(sum(sum(dT*(uF_seq(end-nx_coarse+2:end,:)-U(end-nx_coarse+2:end,:)).^2)));%
        LInfNormErrorGMRES(initer+1) = norm(U_fine_mat_reduce' - uF_seq_mat',inf);%max(abs(uF_seq(end-nx_coarse+2:end,:)-U(end-nx_coarse+2:end,:)));%
