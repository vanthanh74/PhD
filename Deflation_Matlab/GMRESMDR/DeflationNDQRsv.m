function [P_k,Y_k,C_k,U_k,resvec] = DeflationNDQRsv(A,b,matrices,matInd,methodInd,methodDef,resvec,adaptive_deflation,V,H,npes,k,cycle,maxit,maxcycle,x_0,x_1,r_0,maxdef,tol,method)
...
%         %Deflation by ND svd
%         [P_kND,Y_kND,C_kND,U_kND] = DeflationNDsvd(A,V,H,npes,k,cycle,maxcycle,x_1,r_1,tol,method);
%         
%         
%         %Deflation by svd
%         [P_k,Y_k,C_k,U_k] = Deflationsvd(A,V,H,k,cycle,maxcycle,maxdef,r_1,tol,adaptive_deflation,method);
        ...
        %                 size(H) size(A1)
                         ...
                         Hcols=size(H,2);
                        P_k = NDQRCPSingularValuesApproximation(A,npes,k,Hcols);
        %                 P_k = NDQRCPSingularValuesApproximation(H,npes,k);
        %                 P_k1 = P_k1(:, end - k + 1 : end);
        %                 return
        %                 P_k = P_k(:, end - k + 1 : end);
        % size(V(:, 1 : maxit))  1821,25
        % size(P_k)              1821,5  ---> should be 25,5
                        Y_k = V(:, 1 : maxit) * P_k;
        % %                 [Q, R] = qr(A * blcLS(Pr, begin_in, end_in, Y_k), 0);
        %                 [Q, R] = qr(A * Y_k, 0);
                        %----------------------------------
                        % set the approximate right null space of A as the
                        % deflation subspace
        %                 Y_k =  P_k;
    ...
    while(cycle < maxcycle && norm(r_1) > tol)
        ...
        %                 [~, indices] = sort(diag(Eig));
                        Vcols=size(Vtilde,2);
                        P_k = NDQRCPSingularValuesApproximation(A,npes,k,Vcols);
        ...
        %                 P_k = P_k(:, indices(1 : k));
        % size(Vtilde)  317,23
        % size(P_k)     25,5          1821,5  ---> should be 25,5
                        Y_k = Vtilde * P_k;
                        [Q, R] = qr(G * P_k, 0);
                        C_k = Wtilde * Q;
                        U_k = Y_k / R;
