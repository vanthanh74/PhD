...
matrix='e20r0100.mtx';  %sherman3 N=5005 nz=20033,   saylr4 lshp3466 CurlCurl_0 crystk03.mtx ct20stif.mtx bcsstm39.mtx
A =  mmread(matrix);
% load matlab.mat
% A = sparse(A_F);
% n = 15000;
...
M3  = @(x) defpreconditioner(Q2,R22,blcLS(Pr, begin_in, end_in, x));%defpreconditioner(Q2,R22,x) + blcLS(Pr, begin_in, end_in,x);  % mix
% M3  = @(x) blcLS(Pr, begin_in, end_in, defpreconditioner(Q2,R22,x));%defpreconditioner(Q2,R22,x) + blcLS(Pr, begin_in, end_in,x);  % mix
% M3  = @(x) mixedAdditivePreconditioner(Q2,R22,Pr, begin_in, end_in,x);
