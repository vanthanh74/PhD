...
// -----------------------------------------------------------------
// IVP: u_t = alpha*(u_xx + u_yy) - beta*(u_x + u_y) - gamma*u + f    in [0,L]x[0,L] x (0,T),
...
// Function prototypes
void backwardEuler2D(const Mat &initial, Mat &solution, double dx, double dy, double dt, double alpha, double beta, double gamma, int nt, const Mat &f);
void parareal2D(const Mat &initial, Mat &parareal_solution, double dx, double dy, double dt, double alpha, double beta, double gamma, int nt, const Mat &f, int max_iter, Vec &errors);
...
int main() {
    ...
    backwardEuler2D(initial, fine_solution, dx, dy, dt, alpha, beta, gamma, nt, f);

    // Parareal solution
    Mat parareal_solution(nx, Vec(ny));
    Vec errors;
    int max_iter = 20; // Maximum iterations for Parareal
    parareal2D(initial, parareal_solution, dx, dy, dt, alpha, beta, gamma, nt, f, max_iter, errors);
    ...
    for (int i = 0; i < nx; ++i) {
        for (int j = 0; j < ny; ++j) {
            fine_file << i * dx << " " << j * dy << " " << fine_solution[i][j] << endl;
            parareal_file << i * dx << " " << j * dy << " " << parareal_solution[i][j] << endl;
        }
    }
...
void backwardEuler2D(const Mat &initial, Mat &solution, double dx, double dy, double dt, double alpha, double beta, double gamma, int nt, const Mat &f) {
    ...
    double r_conv_x = beta * dt / dx;
    double r_conv_y = beta * dt / dy;
    ...
    for (int t = 0; t < nt; ++t) {
        ...
        for (int i = 1; i < nx - 1; ++i) {
            for (int j = 1; j < ny - 1; ++j) {
                solution[i][j] = (temp[i][j] + rx * (temp[i - 1][j] - 2.0 * temp[i][j] + temp[i + 1][j]) +
                                  ry * (temp[i][j - 1] - 2.0 * temp[i][j] + temp[i][j + 1]) -
                                  r_conv_x * (temp[i][j] - temp[i - 1][j]) -
                                  r_conv_y * (temp[i][j] - temp[i][j - 1]) +
                                  dt * f[i][j]) /
                                 (1 + r_react);
            }
        }
...
void parareal2D(const Mat &initial, Mat &parareal_solution, double dx, double dy, double dt, double alpha, double beta, double gamma, int nt, const Mat &f, int max_iter, Vec &errors) {
    ...
    Mat correction(nx, Vec(ny));
    parareal_solution = initial;

    for (int k = 0; k < max_iter; ++k) {
        // Coarse solver
        backwardEuler2D(parareal_solution, coarse_solution, dx, dy, dt * 10, alpha, beta, gamma, 1, f);

        // Fine solver
        backwardEuler2D(parareal_solution, fine_solution, dx, dy, dt, alpha, beta, gamma, nt, f);
