...
%         [b_U, b_sig, b_V] = svds( 
        ...
        if(k > 0)
            ...
            c = normr * e_1;
            for j = 1 : maxit
                w = A * blcLS(Pr, begin_in, end_in, V(:, j)); % A*Pr^-1*V_j
                temp = V(:, 1 : j)' * w;                  % V_j^T*A*Pr^-1*V_j
                w = w - V(:, 1 : j) * temp; % A*Pr^-1*V_j - V_j*V_j^T*A*Pr^-1*V_j
                temp1 = V(:, 1 : j)' * w; % V_j^T*( A*Pr^-1*V_j - V_j*V_j^T*A*Pr^-1*V_j )
                w = w - V(:, 1 : j) * temp1; % A*Pr^-1*V_j - V_j*V_j^T*A*Pr^-1*V_j - V_j*V_j^T*( A*Pr^-1*V_j - V_j*V_j^T*A*Pr^-1*V_j )
                H(1 : j, j) = temp + temp1; % V_j^T*A*Pr^-1*V_j + V_j^T*( A*Pr^-1*V_j - V_j*V_j^T*A*Pr^-1*V_j )
                H(j + 1, j) = norm(w);
                V(:, j + 1) = w / H(j + 1, j);
