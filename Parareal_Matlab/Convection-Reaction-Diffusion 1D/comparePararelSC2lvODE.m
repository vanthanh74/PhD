...
L2NormError = norm(uF_true(pick)-U_fine(pick),2);%sqrt(sum(Dt*(uF_true(pick)-U_fine(pick)).^2));%
...
FC_boundL2 = (abs(phi^m - phiDT)*(1 - abs(phiDT)^((N)/m) )/ (1 - abs(phiDT)))^(0)*norm(uF_true(pick)-U0n(pick),2);%sqrt(sum(Dt*(uF_true(pick)-U0n(pick)).^2));%
...
%         i = i + 1
  ...
  % second level ~ coarse grid correction sequentially
    ...
    L2NormErrorNonO2(k+1) = norm(uF_true(pick)-U_fine(pick),2);%sqrt(sum(Dt*(uF_true(pick)-U_fine(pick)).^2));%
    L2NormErrorParareal(k+1) = norm(uF_true(pick)-U_Parareal,2);%sqrt(sum(Dt*(uF_true(pick)-U_Parareal).^2));%
    
    LInfNormErrorNonO2(k+1) = norm(uF_true(pick)-U_fine(pick),inf);%max(abs(uF_true(pick)-U_fine(pick)));%
    LInfNormErrorParareal(k+1) = norm(uF_true(pick)-U_Parareal,inf);%max(abs(uF_true(pick)-U_Parareal));%
    ...
    %FC bound
    FC_boundL2(k+1) = (abs(phi^m - phiDT)*(1 - abs(phiDT)^((N)/m) )/ (1 - abs(phiDT)))^(k)*norm(uF_true(pick)-U0n(pick),2);%sqrt(sum(Dt*(uF_true(pick)-U0n(pick)).^2));%
