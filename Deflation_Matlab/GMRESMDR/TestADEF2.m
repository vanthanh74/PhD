...
M1 = @(x) P'*defpreconditioner(Q2,R22,x) + QQ*x;%defpreconditioner(Q2,R22,x);   % deflation preconditioner
M2 = @(x) blcLS(Pr, begin_in, end_in, x); % block jacobi preconditioner
M3  = @(x) P'*defpreconditioner(Q2,R22,x) + QQ*x + blcLS(Pr, begin_in, end_in, x);%defpreconditioner(Q2,R22,x) + blcLS(Pr, begin_in, end_in,x);  % mix
%---------------------------------------------------------
maxit=length(b);
tol=1e-6;
