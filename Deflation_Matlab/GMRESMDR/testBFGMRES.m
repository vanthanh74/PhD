...
for k=1:maxit
    ...
    if cond(H_hat(:,1:k)) > 10^(2*p)/tol || norm( x_k-x_0) < N*norm(x_k)
        U(:,p+1) = V(:,k);
        ...
        %         while cond(H_hat(:,1:k)) > 10^(2*p)/tol
        i=i+1
...
x_gmres = gmres(A,b);
normX_gmres_X = norm(x_gmres-x_k)
normX_Xexact = norm(x_exact-x_k)
normXexact_X_gmres = norm(x_gmres-x_exact)
